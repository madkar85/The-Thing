...
namespace MiracleMileAPI.Calculations
{
    public class Enskildfirma
    {
        ...
        ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        // Utrakning Enskild Firma Med schablonbeskattning, for att forenkla deklarationen.
        ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

        public string Calc_Enskild_Firma(Double lon_a_skatt, Double redan_inbetald_skatt, Double firma_Utgifter, Double firma_intake)
        {

            Double sek = 0.25 * (firma_intake - firma_Utgifter); ;
            Double Overskott = (firma_intake - firma_Utgifter - sek);
            ...
            var vars = new Company();

            vars.Ditt_overskott = Overskott;
            vars.Lon_A_skatt = lon_a_skatt;

            vars.Din_arsinkomst = Math.Round(gl);
            vars.Din_Genomsnittliga_Manadsinkomst_Brutto = Math.Round(gl / 12);

            vars.Beskattningsbar_Forvarvsinkomst = Math.Round(gl - grundavdrag);
            vars.Kommunal_inkomstskatt = Math.Round(kommunal_inkomstskatt);
            vars.Statlig_skatt = Math.Round(statlig_skatt);
            vars.Kvar_till_statlig_skatt = -(lon_till_utbetalning - grundavdrag) + tak_statlig_skatt;
            // Inklusive begravningsavgift.

            vars.Egenavgifter_tot = Math.Round(egenavgifter);
            vars.Egenavgifter_innan_nedsattning = Math.Round(0.2897 * Overskott);
            vars.Egenavgifter_nedsattning = Math.Min(0.075 * Overskott, 0.075 * 200000);
            vars.Kvar_till_okning_av_egenavgiften = Math.Round(200000 - Overskott);

            vars.Grundavdrag = Math.Round(grundavdrag);
            //
            vars.Schablonavdrag_som_maste_tas_upp_som_inkomst_nasta_ar = Math.Round(sek);

            vars.Slutgiltlig_skatt = Math.Round(Slutgiltlig_skatt);
            //vars.Total skatt att betala                  = Math.Round(gl - Lon_efter_skatt + sek);   

            vars.Skatt_kvar_att_betala = Math.Round(Slutgiltlig_skatt - redan_inbetald_skatt);

            vars.Lon_efter_skatt = Math.Round((Lon_efter_skatt) / 12);

            vars.Genomsnittlig_skatt_per_manad = Math.Round(Slutgiltlig_skatt / 12);
            ...
            vars.Effektiv_skatt_pct = Math.Round(100 * (1 - Lon_efter_skatt / (gl + sek)), 2);
